...
import { HotelCard, Navbar,Categories ,SearchStayWithDate,Filter,AuthModal,ProfileDropDown} from "../../components";
import "./Home.css";
...
export const Home = () => {
  ...
  useEffect(() => {
      (async () => {
        try{
          const {data} = await axios.get(`https://travel-app-backend-rbzs.onrender.com/api/hotels?category=${hotelCategory}`
 ...
 const fetchMoreData =  () => {
      ...
      const filteredHotelsByRoomsAndBeds = getHotelsByRoomsAndBeds(filteredHotelByPrice,noOfBathrooms,noOfBedrooms,noOfBeds);
